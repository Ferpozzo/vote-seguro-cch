...
import { elections } from '../constants/votations';

function checkVotation(votation, navigation) {
  let buttonLayout;
  if (votation.status == 'open') {
    buttonLayout = (
  ...
  } else if (votation.status == 'closed') {
    buttonLayout = (
      <Block style={styles.buttons}>
...
class Home extends React.Component {
  renderVotations = () => {
    ...
    let votationsLayout = elections.map((e, index) => {
      return <Block style={styles.votation} shadow card center space={'evenly'} key={index}>
